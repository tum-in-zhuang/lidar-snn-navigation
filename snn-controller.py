...
class SpikingNeuralNetwork:
    def __init__(self):
        ...
        self.spike_generators = nest.Create(
            "poisson_generator",
            p.resolution[0] * p.resolution[1],
            params=p.poisson_params,
        )
        self.neuron_pre = nest.Create(
            "parrot_neuron", p.resolution[0] * p.resolution[1]
        )
        ...
        self.spike_detector = nest.Create(
            "spike_detector", 2, params={"withtime": True}
        )
        ...
        nest.Connect(
            self.neuron_pre, self.neuron_post, "all_to_all", syn_spec=self.syn_dict
        )
    ...
    def simulate(self, dvs_data, reward):
        ...
        # Get network weights
        weights_l = np.array(nest.GetStatus(self.conn_l, keys="weight")).reshape(
            p.resolution
        )
        weights_r = np.array(nest.GetStatus(self.conn_r, keys="weight")).reshape(
            p.resolution
        )
